...
import { getCalendarEventsWithRefresh } from '../services/calendar';
...
export function createCalendarRouter(userRepository: IUserRepository): Router {
  ...
  router.get('/analysis', ensureAuthenticated, async (req: any, res: any) => {
    try {
      ...
      // Get full user data including refresh token
      const user = await userRepository.findByGoogleId(sessionUser.id);
      if (!user) {
        return res.status(401).json({ error: 'User not found' });
      }
      
      // Get calendar events from the past month with refresh token support
      const events = await getCalendarEventsWithRefresh({
      ...
      const workweek = await getUserWorkweek(sessionUser.id);
  ...
  router.get('/week-over-week', ensureAuthenticated, async (req: any, res: any) => {
    try {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        // Get calendar events for this week with refresh token support
        const events = await getCalendarEventsWithRefresh({
          accessToken: sessionUser.accessToken,
          refreshToken: user.refreshToken,
          id: user.googleId
...
// For backward compatibility - export a simple router
